/**
 * Script pour ajouter automatiquement la référence au fichier CSS d'accessibilité
 * à tous les fichiers HTML d'un projet
 * 
 * Usage: 
 * 1. Installez Node.js si ce n'est pas déjà fait
 * 2. Sauvegardez ce script en tant que add-accessibility-css.js à la racine de votre projet
 * 3. Exécutez la commande: node add-accessibility-css.js
 */
...
const cssLinkToAdd = '<link rel="stylesheet" href="styles/accessibilite.css">';
...
/**
 * Vérifie si un fichier HTML contient déjà la référence au CSS d'accessibilité
 * @param {string} content - Contenu du fichier HTML
 * @returns {boolean} - True si la référence existe déjà
 */
function hasAccessibilityCssLink(content) {
    return content.includes('href="styles/accessibilite.css"') || 
           content.includes("href='styles/accessibilite.css'");
}

/**
 * Ajoute la référence au CSS d'accessibilité dans la section head d'un fichier HTML
 * @param {string} content - Contenu du fichier HTML
...
function processHtmlFile(filePath) {
    try {
        ...
        if (hasAccessibilityCssLink(content)) {
            console.log(`  ✓ CSS d'accessibilité déjà présent`);
            filesAlreadyHaveCss++;
            return;
        }
        ...
        console.log(`  ✓ CSS d'accessibilité ajouté`);
        filesModified++;
...
console.log('=== Ajout du CSS d\'accessibilité à tous les fichiers HTML ===');
